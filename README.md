...
`test/TestFacelets.flix` does exactly that. Its expected strings come from
`BigCube.showFacelets` after applying a named turn in that project, by an engine
sharing no code with this one, and they agree at both the 4x4x4 and the 5x5x5.
...
- It does not say whether a state is **reachable**. The format admits
  ...
  bug in the adapter.
- There is no `fromFacelets` yet. Reading facelets back is the harder direction
  and carries a real asymmetry: a project like `flix-cube` can turn a middle
  layer, which moves an odd cube's fixed centres, and Orbit64 has nowhere to put
  that -- those centres are its unencoded frame. Such a state can be *drawn*
  from facelets but not *encoded* without first restoring the frame, so the
  adapter has to reject it or normalise it, and which of those is right is not
  yet decided.
...
- `test/TestFacelets.flix` -- cross-project fixtures at the facelet layer,
  generated by `flix-cube`'s turn engine.
